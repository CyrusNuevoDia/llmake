...
&nbsp;&nbsp;&nbsp;&nbsp;Shell command containing `{prompt}` that swaps
the configured runner for a single invocation. When set, `lens` uses a
plain non-login `/bin/bash` so the command does not depend on user
rc-file success. Useful for tests and ops.
