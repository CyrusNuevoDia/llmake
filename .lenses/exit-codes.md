...
| 2    | Config missing or invalid   | `.lenses/config.yaml` (or `.jsonc`/`.json`) could not be discovered, could not be read, or failed Zod schema validation. Returned before any runner invocation or lockfile write.                                               |
