...
| --------------- | ----- | ------- | ---------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `--config`      | `-c`  | string  | _(unset)_  | global (every verb)                                | Override config discovery. When unset, `lens` searches `.lenses/config.yaml`, then `.jsonc`, then `.json`. |
...
| `--force`       | `-f`  | boolean | `false`    | `init`, `sync`, `pull`                             | `init`: overwrite an existing `.lenses/config.yaml`. `sync`/`pull`: run even when hashes show no drift. Ignored elsewhere. |
